<template>
  <div>
    <ReusTable
      :header="['', 'Icon', 'Achievement name', 'Created', 'Status', 'Action']"
    >
      <template #table>
        <tr v-for="partner in slider" :key="partner.id">
          ...
          <td
            @click="
              router.push({ name: 'SuccessInfo', params: { id: partner.id } })
            "
          >
            <img
              class="mx-2"
...
const emit = defineEmits(["editAchievements"]);
watch(
  () => filteredData.value,
  async (newVal) => {
    if (newVal.length > 0 && newVal[0].slider_type == "achievements ") {
      slider.value = newVal;
    } else {
      await sliderStore().getSingleSliderType("achievements");
    }
  }
);
onMounted(async () => {
...
const toggleStatus = async (id, e) => {
  if (e.target.checked) {
    const res = await sliderStore().restoreSlider({ id: id }, "achievements");
    if (!res) {
      e.target.checked = !e.target.checked;
    }
  } else {
    const res = await sliderStore().destroySlider({ id: id }, "achievements");
    if (!res) {
      e.target.checked = !e.target.checked;
    }
  }
...
const editPartner = async (partnerId) => {
  ...
  if (res) {
    emit("editAchievements", singleSlide.value);
  }
};
</script>
