<template>
  <div>
    <ReusTable
      :header="['', 'Image', 'Value Name', 'Created', 'Status', 'Action']"
    >
      <template #table>
        <tr v-for="val in slider" :key="val.id">
          <td>
            <div class="form-check form-switch">
              <input
                :checked="!val.deleted_at"
                @change="toggleStatus(val.id, $event)"
          ...
          <td
            @click="
              router.push({ name: 'our-value-info', params: { id: val.id } })
            "
          >
            <img
              class="mx-2"
          ...
          <td>
            {{ moment(new Date(val.created_at)).format("DD-MM-YYYY") }}
          </td>
          ...
          >
            {{ val.deleted_at == null ? "Active" : "Suspended" }}
          </td>
...
watch(
  () => filteredData.value,
  async (newVal) => {
    if (newVal.length > 0 && newVal[0].slider_type == "our_values") {
      slider.value = newVal;
    } else {
      await sliderStore().getSingleSliderType("our_values");
    }
  }
);
onMounted(async () => {
...
const toggleStatus = async (id, e) => {
  if (e.target.checked) {
    const res = await sliderStore().restoreSlider({ id: id }, "our_values");
    if (!res) {
      e.target.checked = !e.target.checked;
    }
  } else {
    const res = await sliderStore().destroySlider({ id: id }, "our_values");
    if (!res) {
      e.target.checked = !e.target.checked;
    }
  }
...
const editValues = async (pack) => {
  let res = await sliderStore().getSlide({ id: pack });
  if (res) {
    emit("editValue", singleSlide.value);
  }
};
</script>
