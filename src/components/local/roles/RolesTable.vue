<template>
  <div>
    <ReusTable :header="['', 'Role Name', 'Status', 'Created', 'Action']">
      <template #table>
        <tr v-for="rol in roles" :key="rol.id">
          ...
          <!-- <td v-if="!rol.deleted_at" style="color: #34c759 !important">
            Active
          </td>
          <td v-else>Suspended</td> -->
          <td>status</td>
          <td>
            {{ moment(new Date(rol.created_at)).format("DD-MM-YYYY") }}
          </td>
          <td style="width: 15%">
...
const emit = defineEmits(["editRole"]);
...
//   if (e.target.checked) {
//     const res = await roleStore().restoreCategory({ id: id });
//     if (!res) {
//       e.target.checked = !e.target.checked;
//     }
//   } else {
//     const res = await roleStore().destroyCategory({ id: id });
//     if (!res) {
//       e.target.checked = !e.target.checked;
//     }
//   }
//   await roleStore().getActiveCategory();
// };

const removeRole = async (id) => {
  const res = await roleStore().destroyRole({ id: id });

  await roleStore().getAllRoles();
};

const editRole = async (roleId) => {
  let res = await roleStore().showRole({ id: roleId });
  if (res) {
    emit("editRole", role.value);
  }
};
</script>
