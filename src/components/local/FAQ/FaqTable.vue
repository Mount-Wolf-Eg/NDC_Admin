<template>
  <div>
    <ReusTable
      :header="['', 'Question', 'Answer', 'Status', 'Created', 'Action']"
    >
      <template #table>
        <tr v-for="(faq, i) in allQuestions" :key="i">
          <td>
            <div class="form-check form-switch">
              <input
                :checked="!faq.deleted_at"
                @change="toggleStatus(faq.id, $event)"
          ...
          >
            {{ faq.deleted_at == null ? "Active" : "Suspended" }}
          </td>

          <td>
            {{ moment(new Date(faq.created_at)).format("DD-MM-YYYY") }}
          </td>
          <td style="width: 15%">
...
const toggleStatus = async (id, e) => {
  if (e.target.checked) {
    const res = await FAQStore().restoreQuestion({ id: id });
    if (!res) {
      e.target.checked = !e.target.checked;
    }
  } else {
    const res = await FAQStore().destroyQuestion({ id: id });
    if (!res) {
      e.target.checked = !e.target.checked;
    }
  }
...
const removeQuestion = async (id) => {
  const res = await FAQStore().deleteQuestion({ id: id });
  await FAQStore().getAllQuestions();
};

const editQuestion = async (questionId) => {
  let res = await FAQStore().getQuestion({ id: questionId });
  if (res) {
