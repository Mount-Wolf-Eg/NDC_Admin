<template>
  <div>
    ...
    >
      <template #table>
        <tr v-for="serv in allServices" :key="serv.id">
          <td>
            <div class="form-check form-switch">
              <input
                :checked="!serv.deleted_at"
                @change="toggleStatus(serv.id, $event)"
          ...
          <td
            @click="
              router.push({ name: 'serviceInfo', params: { id: serv.id } })
            "
          >
            <img
              class="mx-2"
          ...
          <td>
            {{ moment(new Date(serv.created_at)).format("DD-MM-YYYY") }}
          </td>

          <td
            :style="`${
              serv.deleted_at == null
                ? 'color: var(--col-success) !important'
                : 'color: var(--col-error) !important'
            }`"
          >
            {{ serv.deleted_at == null ? "Active" : "Suspended" }}
          </td>
...
const toggleStatus = async (id, e) => {
  if (e.target.checked) {
    const res = await useServiceStore().restoreService({ id: id });
    if (!res) {
      e.target.checked = !e.target.checked;
    }
  } else {
    const res = await useServiceStore().destroyService({ id: id });
    if (!res) {
      e.target.checked = !e.target.checked;
    }
  }
...
const removeService = async (id) => {
  const res = await useServiceStore().deleteService({ id: id });
  await useServiceStore().getAllServices();
};

const editService = async (partnerId) => {
  let res = await useServiceStore().getService({ id: partnerId });
  if (res) {
    emit("editMainService", service.value);
  }
};
</script>
