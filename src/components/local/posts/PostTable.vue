<template>
  <div>
    <ReusTable
      :header="['', 'Post Title', 'Category', 'Created', 'Status', 'Action']"
    >
      <template #table>
        <tr v-for="(post, i) in allPosts" :key="i">
          <td>
            <div class="form-check form-switch">
              <input
                :checked="!post.deleted_at"
                @change="toggleStatus(post.id, $event)"
          ...
          <td
            @click="router.push({ name: 'postInfo', params: { id: post.id } })"
          >
            {{ post.title }}
          </td>
          ...
          >
            {{ post.deleted_at == null ? "Active" : "Suspended" }}
          </td>
...
const toggleStatus = async (id, e) => {
  if (e.target.checked) {
    const res = await usePostStore().restorePost({ id: id });
    if (!res) {
      e.target.checked = !e.target.checked;
    }
  } else {
    const res = await usePostStore().destroyPost({ id: id });
    if (!res) {
      e.target.checked = !e.target.checked;
    }
  }
...
const removeQuestion = async (id) => {
  const res = await usePostStore().deletePost({ id: id });
  await usePostStore().getAllPosts();
};

const editCateg = async (postID) => {
  let res = await usePostStore().getPost({ id: postID });
  if (res) {
    emit("editPost", post.value);
  }
};
</script>
