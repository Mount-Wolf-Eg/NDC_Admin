        ...
        <tr v-for="job in allJobs" :key="job.id">
          <td>
            <div class="form-check form-switch">
              <input
                :checked="!job.deleted_at"
                @change="toggleStatus(job.id, $event)"
          ...
          <td
            @click="router.push({ name: 'JobInfo', params: { id: job.id } })"
          ...
          >
            {{ job.deleted_at == null ? "Active" : "Suspended" }}
          </td>
...
const toggleStatus = async (id, e) => {
  if (e.target.checked) {
    const res = await useJobsStore().restoreJob({ id: id });
    if (!res) {
      e.target.checked = !e.target.checked;
    }
  } else {
    const res = await useJobsStore().destroyJob({ id: id });
    if (!res) {
      e.target.checked = !e.target.checked;
    }
  }
...
const removeJob = async (id) => {
  const res = await useJobsStore().deleteJob({ id: id });
  await useJobsStore().getAllJobs();
};

const editJob = async (jobId) => {
  let res = await useJobsStore().getJob({ id: jobId });
  if (res) {
    emit("editJob", job.value);
  }
};
</script>
