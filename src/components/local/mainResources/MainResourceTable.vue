...
const emit = defineEmits(["editMainResource"]);
watch(
  () => filteredData.value,
  async (newVal) => {
    if (newVal.length > 0 && newVal[0].slider_type == "main_resources") {
      slider.value = newVal;
    } else {
      await sliderStore().getSingleSliderType("main_resources");
    }
  }
);
onMounted(async () => {
...
const toggleStatus = async (id, e) => {
  if (e.target.checked) {
    const res = await sliderStore().restoreSlider({ id: id }, "main_resources");
    if (!res) {
      e.target.checked = !e.target.checked;
    }
  } else {
    const res = await sliderStore().destroySlider({ id: id }, "main_resources");
    if (!res) {
      e.target.checked = !e.target.checked;
    }
  }
...
const editPartner = async (partnerId) => {
  ...
  if (res) {
    emit("editMainResource", singleSlide.value);
  }
};
</script>
