    ...
    >
      <template #table>
        <tr v-for="partner in slider" :key="partner.id">
          ...
          <td style="width: 30%; white-space: wrap; text-overflow: ellipsis">
            {{ partner.description }}
          </td>
          <td>
...
const toggleStatus = async (id, e) => {
  if (e.target.checked) {
    const res = await sliderStore().restoreSlider({ id: id }, "testimonials");
    if (!res) {
      e.target.checked = !e.target.checked;
    }
  } else {
    const res = await sliderStore().destroySlider({ id: id }, "testimonials");
    if (!res) {
      e.target.checked = !e.target.checked;
    }
  }
...
const editPartner = async (partnerId) => {
  ...
  if (res) {
    emit("editCustomers", singleSlide.value);
  }
};
</script>
