    ...
    <ReusTable :header="['', 'Partner Name', 'Created', 'Status', 'Action']">
      <template #table>
        <tr v-for="partner in slider" :key="partner.id">
          ...
          <td
            @click="
              router.push({ name: 'partner-data', params: { id: partner.id } })
            "
          >
            <img
              class="mx-2"
...
const toggleStatus = async (id, e) => {
  if (e.target.checked) {
    const res = await sliderStore().restoreSlider({ id: id }, "partners");
    if (!res) {
      e.target.checked = !e.target.checked;
    }
  } else {
    const res = await sliderStore().destroySlider({ id: id }, "partners");
    if (!res) {
      e.target.checked = !e.target.checked;
    }
  }
